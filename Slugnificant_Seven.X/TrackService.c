/*
 * File: Service.c
 * Author: J. Edward Carryer
 * Modified: Gabriel H Elkaim
 * 
 * Project: Slugnificant Seven
 * Modified: Jian Hao Miao, Evan Plummer, Stephanie Lin
 */

/*******************************************************************************
 * MODULE #INCLUDE                                                             *
 ******************************************************************************/
#include <stdio.h>
#include <stdint.h>
#include "BOARD.h"
#include "AD.h"
#include "ES_Configure.h"
#include "ES_Framework.h"
#include "TrackService.h"

/*******************************************************************************
 * MODULE #DEFINES                                                             *
 ******************************************************************************/
#define TIMER_TICKS 5 //20
#define NUM_EVENTS 10

#define FRONT_THRESHOLD   860
#define BACK_THRESHOLD    850

/*******************************************************************************
 * PRIVATE FUNCTION PROTOTYPES                                                 *
 ******************************************************************************/

/*******************************************************************************
 * PRIVATE MODULE VARIABLES                                                    *
 ******************************************************************************/
static uint8_t MyPriority;

/*******************************************************************************
 * PUBLIC FUNCTIONS                                                            *
 ******************************************************************************/
uint8_t InitTrackService(uint8_t Priority) {
    ES_Event ThisEvent;

    MyPriority = Priority;
    ES_Timer_InitTimer(TRACK_SERVICE_TIMER, TIMER_TICKS); // set timer

    // initialize pins
    //    AD_AddPins(FRONT_IN | BACK_IN);

    // post the initial transition event
    ThisEvent.EventType = ES_INIT;
    if (ES_PostToService(MyPriority, ThisEvent) == TRUE) {
        return TRUE;
    } else {
        return FALSE;
    }
}

uint8_t PostTrackService(ES_Event ThisEvent) {
    return ES_PostToService(MyPriority, ThisEvent);
}

ES_Event RunTrackService(ES_Event ThisEvent) {
    ES_Event ReturnEvent;
    ReturnEvent.EventType = ES_NO_EVENT; // assume no errors

    static uint8_t eventHistory[NUM_EVENTS];
    static uint8_t prevEventParam;
    uint8_t currEventParam, i;

    // service code starts here
    switch (ThisEvent.EventType) {
        case ES_INIT:
            break;
        case ES_NO_EVENT:
        case ES_TIMERACTIVE:
            break;
        case ES_TIMEOUT:
            currEventParam = 0;
                // printf("%d\r\n", AD_ReadADPin(TRACK_FRONT_IN));
            
            if (AD_ReadADPin(TRACK_FRONT_IN) >= FRONT_THRESHOLD) {
                currEventParam |= FRONT_TRACK;
                // printf("%d\r\n", AD_ReadADPin(FRONT_IN));
            }
            if (AD_ReadADPin(TRACK_BACK_IN) >= BACK_THRESHOLD) {
                currEventParam |= BACK_TRACK;
                // printf("%d\r\n", AD_ReadADPin(BACK_IN));
            }
            // Check if event is valid
            uint8_t validEvent = TRUE;
            for (i = 0; i < NUM_EVENTS; i++) {
                if (currEventParam != eventHistory[i]) {
                    validEvent = FALSE;
                    break;
                }
            }
            // check if there is an event change
            if ((currEventParam != prevEventParam) & validEvent) {
                ReturnEvent.EventType = TRACK_EVENT;
                ReturnEvent.EventParam = currEventParam;
                prevEventParam = currEventParam; // update history
#ifndef SERVICE_TEST // for test harness
                PostHSM(ReturnEvent);
#else
                PostTrackService(ReturnEvent);
#endif   
            }
            // update history
            for (i = NUM_EVENTS - 1; i > 0; i--) {
                eventHistory[i] = eventHistory[i - 1];
            }
            eventHistory[0] = currEventParam;
            ES_Timer_InitTimer(TRACK_SERVICE_TIMER, TIMER_TICKS);
            break;
#ifdef SERVICE_TEST // for test harness   
        default:
//            printf("\r\nEvent: %s\tParam: 0x%X",
//                    EventNames[ThisEvent.EventType], ThisEvent.EventParam);
            break;
#endif
    }
    return ReturnEvent;
}

/*******************************************************************************
 * PRIVATE FUNCTIONs                                                           *
 ******************************************************************************/

